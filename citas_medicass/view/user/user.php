...
<?php if(count($data)>0):?>
    <?php foreach($data as $d):?>
								<div class="card-body">
									...
									<form action="mail/autenticar.php" method="post">	
									<div class="col-md-6">
										...
										$email_exist = $query->fetch_assoc();

										if (isset( $email_exist['email'])) {
											# code...
											$autenticado = $email_exist['email'];
										}
										...
										/// hacer un query para verificar si el email existe o no


										

										if(isset($autenticado) && $autenticado == true){

											echo "Autenticado"; 

										}else{
											echo '<p class="text-danger">! Debe autenticar con su correo electronico para poder solicitar una Cita medica y/o cancelar.
										...
										<div class="form-group form-group-default">
											<label for="email">Autenticar email</label>
										...
										<?php
											 	if (!empty($autenticado) != '') {
													...
													echo '<button class="btn btn-primary">Autenticar</button>
													';
												}
											 ?>
